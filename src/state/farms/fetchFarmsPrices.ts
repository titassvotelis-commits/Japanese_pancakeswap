...
import { fetchBnbUsdPrice } from 'utils/fetchBnbUsdPrice'
...
async function fetchBnbPriceBusdFallback(): Promise<BigNumber> {
  const p = await fetchBnbUsdPrice()
  return p > 0 ? new BigNumber(p) : BIG_ZERO
}

const fetchFarmsPrices = async (farms: SerializedFarm[]) => {
  const bnbBusdFarm = farms.find((farm) => farm.pid === 2)
  let bnbPriceBusd = bnbBusdFarm?.tokenPriceVsQuote ? BIG_ONE.times(bnbBusdFarm.tokenPriceVsQuote) : BIG_ZERO
  if (bnbPriceBusd.isZero()) {
    bnbPriceBusd = await fetchBnbPriceBusdFallback()
  }
