...
import { KNOWN_LOCAL_PAIR_ADDRESSES, LOCAL_FACTORY } from 'utils/swap/constants'
import { getLocalLpToken } from 'utils/liquidityPair'
...
export function useThemeManager(): [boolean, () => void, (isDark: boolean) => void] {
  const dispatch = useDispatch<AppDispatch>()
  ...
  const setTheme = useCallback(
    (nextIsDark: boolean) => {
      dispatch(setThemeAction({ isDark: nextIsDark }))
    },
    [dispatch],
  )
...
export function toV2LiquidityToken([tokenA, tokenB]: [Token, Token]): Token {
  const localAddress = computePairAddressFromConfig(tokenA, tokenB, LOCAL_FACTORY)
  const knownLocal = KNOWN_LOCAL_PAIR_ADDRESSES[tokenA.chainId]?.includes(localAddress.toLowerCase())
  if (knownLocal) {
    return getLocalLpToken(tokenA.chainId, localAddress)
  }
