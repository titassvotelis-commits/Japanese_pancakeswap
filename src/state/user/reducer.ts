...
export interface UserState {
  ...
  audioPlay: boolean
  isDark: boolean
  /** Bumped when site-wide theme defaults change (see updateVersion). */
...
export default createReducer(initialState, (builder) =>
  builder
    .addCase(updateVersion, (state) => {
      ...
      // One-time: dark theme is the site default for new and existing users
      if ((state.themeDefaultVersion ?? 0) < 1) {
        state.isDark = true
        state.themeDefaultVersion = 1
      }
    ...
    .addCase(setTheme, (state, { payload: { isDark } }) => {
      state.isDark = isDark
    })
