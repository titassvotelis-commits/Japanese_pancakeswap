...
import { isConfiguredContractAddress, getPancakeProfileAddress } from 'utils/addressHelpers'
...
const getProfileContractOrNull = () => getProfileContract()

const noProfileContractResponse = (): GetProfileResponse => ({
  hasRegistered: false,
  profile: null,
})
...
export const getProfileAvatar = async (address: string) => {
  try {
    const profileContract = getProfileContractOrNull()
    if (!profileContract) {
...
export const getProfile = async (address: string): Promise<GetProfileResponse> => {
  try {
    if (!isConfiguredContractAddress(getPancakeProfileAddress())) {
      return noProfileContractResponse()
    }
    const profileContract = getProfileContractOrNull()
    if (!profileContract) {
      return noProfileContractResponse()
    ...
    if (isActive) {
      ...
      Cookies.set(
        ...
        {
          username,
          avatar: `https://optimuswap.io/images/nfts/${nft?.images.sm}`,
        },
        { domain: 'optimuswap.io', secure: true, expires: 30 },
      )
    }
