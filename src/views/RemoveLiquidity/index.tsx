...
export default function RemoveLiquidity({
  ...
  const { pair, lpToken, parsedAmounts, error } = useDerivedBurnInfo(currencyA ?? undefined, currencyB ?? undefined)
  ...
  // pair contract
  const pairContract: Contract | null = usePairContract(lpToken?.address)
    ...
    <Page>
      ...
      {pair ? (
        <AutoColumn style={{ minWidth: '20rem', width: '100%', maxWidth: '400px', marginTop: '1rem' }}>
          <MinimalPositionCard showUnwrapped={oneCurrencyIsWETH} pair={pair} lpToken={lpToken} />
        </AutoColumn>
      ) : null}
    </Page>
  )
