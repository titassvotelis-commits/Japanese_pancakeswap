...
import { safeInvertedPriceSignificant, safePriceSignificant } from '../../utils/safeFractionPrice'
  ...
  return (
    <>
      ...
      <RowBetween>
        <Text>{t('Rates')}</Text>
        <Text>
          {`1 ${currencies[Field.CURRENCY_A]?.symbol} = ${safePriceSignificant(price, 4) ?? '-'} ${
      ...
      <RowBetween style={{ justifyContent: 'flex-end' }}>
        <Text>
          {`1 ${currencies[Field.CURRENCY_B]?.symbol} = ${safeInvertedPriceSignificant(price, 4) ?? '-'} ${
