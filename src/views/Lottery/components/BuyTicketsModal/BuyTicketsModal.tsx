...
const BuyTicketsModal: React.FC<BuyTicketsModalProps> = ({ onDismiss }) => {
  ...
  const { balance: userCake, fetchStatus } = useTokenBalance(tokens.jnto.address)
  ...
  useEffect(() => {
    const getMaxPossiblePurchase = () => {
      const maxBalancePurchase = memoisedUserCake.div(priceTicketInOpt)
      const limitedMaxPurchase = limitNumberByMaxTicketsPerBuy(maxBalancePurchase)
      let maxPurchase

      // If the users' max JNTo balance purchase is less than the contract limit - factor the discount logic into the max number of tickets they can purchase
  ...
  useEffect(() => {
    ...
    const costBeforeDiscount = priceTicketInOpt.times(numberOfTicketsToBuy)
  ...
  }, [ticketsToBuy, priceTicketInOpt, discountDivisor, getTicketCostAfterDiscount])
  ...
  const getErrorMessage = () => {
    if (userNotEnoughCake) return t('Insufficient JNTo balance')
  ...
  return (
    <StyledModal title={t('Buy Tickets')} onDismiss={onDismiss} headerBackground={theme.colors.gradients.cardHeader}>
      ...
      <BalanceInput
        ...
        currencyValue={
          cakePriceBusd.gt(0) &&
          `~${ticketsToBuy ? getFullDisplayBalance(priceTicketInOpt.times(new BigNumber(ticketsToBuy))) : '0.00'} JNTo`
        }
      />
      ...
      <Flex flexDirection="column">
        <Flex mb="8px" justifyContent="space-between">
          ...
          <Text color="textSubtle" fontSize="14px">
            {priceTicketInOpt && getFullDisplayBalance(priceTicketInOpt.times(ticketsToBuy || 0))} JNTo
          </Text>
        </Flex>
        <Flex mb="8px" justifyContent="space-between">
          <Flex>
