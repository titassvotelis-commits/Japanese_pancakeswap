...
const PreviousRoundCardFooter: React.FC<{ lotteryNodeData: LotteryRound; lotteryId: string }> = ({
  ...
  if (lotteryNodeData) {
    const { amountCollectedInOpt } = lotteryNodeData
    prizeInBusd = amountCollectedInOpt.times(cakePriceBusd)
  }
      ...
      <>
        ...
        {prizeInBusd.isNaN() ? (
          ...
          <Balance
            ...
            color="textSubtle"
            unit=" JNTo"
            value={getBalanceNumber(lotteryNodeData?.amountCollectedInOpt)}
            decimals={0}
          />
        )}
      </>
