...
const RewardBrackets: React.FC<RewardMatchesProps> = ({ lotteryNodeData, isHistoricRound }) => {
  ...
  useEffect(() => {
    if (lotteryNodeData) {
      const { treasuryFee, amountCollectedInOpt, rewardsBreakdown, countWinnersPerBracket } = lotteryNodeData

      const feeAsPercentage = new BigNumber(treasuryFee).div(100)
      const cakeToBurn = feeAsPercentage.div(100).times(new BigNumber(amountCollectedInOpt))
      const amountLessTreasuryFee = new BigNumber(amountCollectedInOpt).minus(cakeToBurn)
      setState({
        isLoading: false,
        cakeToBurn,
