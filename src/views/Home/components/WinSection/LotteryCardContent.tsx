...
const LotteryCardContent = () => {
  ...
  const cakePrizesText = t('%cakePrizeInUsd% in JNTo prizes this round', { cakePrizeInUsd: cakePriceBusdAsString })
  ...
  useEffect(() => {
    ...
    const fetchCurrentLotteryPrize = async () => {
      const { amountCollectedInOpt } = await fetchLottery(lotteryId)
      const prizeInBusd = cakePriceBusd.times(amountCollectedInOpt)
    ...
    <>
      <Flex flexDirection="column" mt="48px">
        ...
        <Text color="white" mb="40px">
          {t('Buy tickets with JNTo, win JNTo if your numbers match')}
        </Text>
      </Flex>
