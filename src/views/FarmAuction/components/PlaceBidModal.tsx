...
const PlaceBidModal: React.FC<PlaceBidModalProps> = ({
  ...
  useEffect(() => {
    if (userNotEnoughCake) {
      setErrorText(t('Insufficient JNTo balance'))
    } else if (!isMoreThanInitialBidAmount && isFirstBid) {
      setErrorText(t('First bid must be %initialBidAmount% JNTo or more.', { initialBidAmount }))
    ...
    <StyledModal title={t('Place a Bid')} onDismiss={onDismiss} headerBackground={theme.colors.gradients.cardHeader}>
      <ExistingInfo>
        <Flex justifyContent="space-between">
          ...
          <Text>{t('%num% JNTo', { num: getBalanceNumber(amount).toLocaleString() })}</Text>
        </Flex>
        <Flex justifyContent="space-between">
      ...
      <InnerContent>
        ...
        {isFirstBid && (
          <Text pb="8px" small>
            {t('First bid must be %initialBidAmount% JNTo or more.', { initialBidAmount })}
          </Text>
        )}
        <BalanceInput
        ...
        <Text color="textSubtle" small mt="24px">
          {t('If your bid is unsuccessful, you’ll be able to reclaim your JNTo after the auction.')}
        </Text>
      </InnerContent>
    </StyledModal>
  )
