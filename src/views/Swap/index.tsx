...
import { Button, Text, ArrowDownIcon, Box, useModal, useMatchBreakpoints } from '@pancakeswap/uikit'
...
import TradePageHero from '../../components/TradePageHeader'
...
export default function Swap({ history }: RouteComponentProps) {
  ...
  const handleSwap = useCallback(() => {
    ...
    if (!swapCallback) {
      setSwapState({
        ...
        swapErrorMessage:
          swapCallbackError ??
          t('Unable to execute swap. Check your wallet connection and try again.'),
        txHash: undefined,
      })
      return
    }
    ...
    swapCallback()
      ...
      .catch((error) => {
        setSwapState({
          attemptingTxn: false,
          tradeToConfirm,
          swapErrorMessage: error?.message ?? t('Swap failed'),
          txHash: undefined,
        })
      })
  }, [priceImpactWithoutFee, swapCallback, swapCallbackError, tradeToConfirm, t])
  ...
  const handleMaxInput = useCallback(() => {
    const max = maxAmountSpend(currencyBalances[Field.INPUT])
    if (max && JSBI.greaterThan(max.raw, JSBI.BigInt(0))) {
      onUserInput(Field.INPUT, max.toExact())
    }
  ...
  const [onPresentConfirmModal] = useModal(
    <ConfirmSwapModal
      ...
      swapErrorMessage={swapErrorMessage ?? swapCallbackError ?? undefined}
      ...
      >
        <AppBody trade>
          <AppHeader
            ...
            chartToggle={{ isOpen: chartOpen, onToggle: () => setChartOpen((v) => !v) }}
          />
            ...
            <AutoColumn justify="space-between">
              <AutoRow justify={isExpertMode ? 'space-between' : 'center'} style={{ padding: '0 1rem' }}>
                <SwapTokenDirectionButton
                  aria-label={t('Switch tokens')}
                  active={Boolean(currencies[Field.INPUT] && currencies[Field.OUTPUT])}
                  iconSize="22px"
                  onClick={() => {
                    setApprovalSubmitted(false) // reset 2 step UI for approvals
                    onSwitchTokens()
          ...
          <Box mt="1rem">
            {swapIsUnsupported ? (
              ...
              <Button
                ...
                id="swap-button"
                width="100%"
                disabled={!isValid || !trade || (priceImpactSeverity > 3 && !isExpertMode)}
              >
                {swapInputError ||
                  swapCallbackError ||
                  (priceImpactSeverity > 3 && !isExpertMode
